...
void* takeOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
		...
		// Ensures no new orders are put into the buffer unless there is at least one space in it
		if(N == num_new_orders){
			pthread_cond_wait(&cond_notFull, &data_mutex);
		}
...
void* processOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
		...
		// Orders are not retrieved unless their is one new order
		if(num_new_orders == 0){
			pthread_cond_wait(&cond_notEmpty, &data_mutex);	
		}
